...
(function($) {
    ...
    var str_opt_dom = "";
    for (var i = 1; i < 32; i++) {
        if (i == 1 || i == 21 || i == 31) { var suffix = "天"; }
        else if (i == 2 || i == 22) { var suffix = "天"; }
        else if (i == 3 || i == 23) { var suffix = "天"; }
        else { var suffix = "天"; }
        str_opt_dom += "<option value='"+i+"'>" + '第' +i +suffix + "</option>\n";
    }
    ...
    var methods = {
        init : function(opts) {
            ...
            block["period"] = $("<span class='cron-period'>"
                    + "每 <select name='cron-period' class='sel-mon'>" + custom_periods
            ...
            block["month"] = $("<span class='cron-block cron-block-month'>"
                    + " 在 <select name='cron-month'>" + str_opt_month
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
          ...
          if (o.useGentleSelect) select.gentleSelect(o.domOpts);
            ...
            block["time"] = $("<span class='cron-block cron-block-time'>"
                    + " 的 <select name='cron-time-hour' class='cron-time-hour'>" + str_opt_hid
                    + "</select>&nbsp:&nbsp<select name='cron-time-min' class='cron-time-min'>" + str_opt_mih
                    + " </span>")
                .appendTo(this)
                .data("root", this);
