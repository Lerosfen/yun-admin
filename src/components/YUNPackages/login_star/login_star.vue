...
<script>
  ...
  export default {
    ...
    props: {
      point: {
        type: Number,
        default: 20 // 生成的星星（点）的个数, 默认25个
      },
      lineColor: {
        type: String,
        default: 'rgba(3, 253, 254,0.9)' // 线的颜色
      },
      roundColor: {
        type: String,
        default: 'rgba(3, 253, 254,0.9)' // 星星的颜色
      },
    },
    data() {
      return {
    ...
    methods: {
      ...
      drawCircle(context, x, y, r, moveX, moveY) {
        ...
        context.beginPath();
        context.arc(circle.x, circle.y, circle.r / 2, 0, 2 * Math.PI);
        context.closePath();
        context.fill();
        return circle
      },
      ...
      drawLine(context, beginX, beginY, closeX, closeY, opacity) {
        context.beginPath();
        context.strokeStyle = `rgba(3, 253, 254, ${opacity})`;
