    ...
    <div class="content-section">
      ...
      <div class="search-section clearfix">
        <el-button-group>
          ...
          <el-button
            class="yellow-btn"
            @click="$router.push({path: 'cleanStrategyMange',query: {
          menuLink: $route.query.menuLink
        }})"
          >返回</el-button>
        </el-button-group>
      ...
      <div class="table-container pag-position">
          ...
          <el-table-column label="操作" width="160">
            <template slot-scope="scope">
              <el-button
                ...
                type="text"
                size="small"
                v-if="btn.selectCleaningStrategyById==1"
              >查看</el-button>
              <el-button
                @click="handleClick(scope.row,4)"
                type="text"
                size="small"
                v-if="btn.sendCleaningStrategy==1 && scope.row.enableState=='禁用'"
              >启用</el-button>
              <!--<el-button @click="handleClick(scope.row,5)" type="text" size="small" v-if="btn.JyCleaningStrategyById==1 && scope.row.enableState=='启用'">禁用</el-button>-->
              <el-button
        ...
        ></el-pagination>
        <!--策略-->
        <yj-dialog title="数据清洗策略-编辑" :visible.sync="stragtegyFormVisible" width="40%">
...
export default {
  ...
  methods: {
    ...
    getclearstrategyList() {
      api
        .selectAllScriptByPage({ currentPage: "1", pageSize: "10" })
        .then(res => {
    ...
    checkclearstrategyList() {
      api.clearstrategyList(this.cleanrules).then(res => {
        if (res.code === "code_200") {
          if (res.data) {
            res.data.forEach(res => {
              let newTime = new Date(res.createTime);
    ...
    delCleanStragte(id) {
      this.$confirm("确认删除该策略吗？", "提示", {
        ...
        .then(() => {
          api.cleartrategy({ id: id }).then(res => {
            if (res.code === "code_200") {
    ...
    // 启用
    AbleStragte(id) {
      api.ableClean(qs.stringify({ cleaningStrategyId: id })).then(res => {
        if (res.code == "code_200") {
          this.$notify({
            tittle: "成功",
            message: res.msg,
    ...
    // 禁用
    enableStragte(id) {
      api.EnableClean(qs.stringify({ id: id })).then(res => {
        if (res.code == "code_200") {
          this.$notify({
            tittle: "成功",
            message: res.msg,
    ...
    strategySubmit() {
      ...
      api.caozuoStrategy(this.celueEdit).then(res => {
        if (res.code === "code_200") {
          this.$notify({
            tittle: "编辑成功！",
            message: "编辑成功！",
    ...
    removeSubmit() {},
    editSubmit() {
      api.cleartrategy({ id: this.rowId }).then(res => {
        if (res.code === "code_200") {
        } else {
          this.$notify({
            tittle: "失败",
...
@import "cleansStrategy-addH.less";
</style>
